...
- **Auto-enrich (no manual `google_ads_eligible` gate):** On the same PATCH/bulk-update when **`qualification_status`** changes, the server merges **`compute_ads_enrichment_fields`** (`google_ads_export.py`) into the Leads row: **`google_ads_eligible`** **`TRUE`**/**`FALSE`**, identifiers, and empty conversion name/value/currency filled from **`enrich_lead_row`** / **`ads_config()`** (explicit sheet overrides kept). Offline export tab inclusion still requires truthy eligible and not **`google_ads_export_override=exclude`**.
